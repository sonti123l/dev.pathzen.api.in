...
class RoomController {
    async createLiveInputForTeacher({ subModuleId, payload, }) {
        let statusCode;
        ...
        // call cloudflare
        const { liveInputId, streamKey, hlsUrl, iframeUrl, } = await createLiveInput(payload?.title);
        // store data into DB
    ...
    async getActiveLive({ subModuleId }) {
        let statusCode;
        let statusCodeMessage;
        let resultResponse;
        ...
        setImmediate(async () => {
            try {
                ...
                await db
                    ...
                    .set({ recordingUrl })
                    .where(eq(rooms.id, roomId));
                console.log(`[RoomController] Recording URL saved for room ${roomId}:`, recordingUrl);
            }
            catch (err) {
                console.error(`[RoomController] Failed to fetch recording URL for room ${roomId}:`, err);
            }
        });
        statusCode = StatusCodes.OK;
        statusCodeMessage = getStatusMessage(statusCode);
