...
class resourceController {
    async getColleges({ page, limit, search }) {
        const offset = (page - 1) * limit;
        const cleanSearch = search?.trim();
        const whereCondition = cleanSearch && cleanSearch.length > 0
    ...
    async getCoursesAccordingToId(domainId) {
    ...
    async getCourseDetailsByCourseId(course_id) {
        ...
        const getModulesData = await db
            ...
            .from(modules)
            .leftJoin(subModules, eq(modules.module_id, subModules.sub_module_in_module_id))
        ...
        if (getCourseDetails?.length > 0) {
            ...
            if (getDomainDetails) {
                if (getModulesData?.length > 0) {
                    const getModuleEntireFormattedData = await arrangeData(getModulesData);
                    statusCode = StatusCodes.OK;
                    statusCodeMessage = getStatusMessage(statusCode);
                    result = createDataSchemaAndReturnIt({
                        status: statusCode,
