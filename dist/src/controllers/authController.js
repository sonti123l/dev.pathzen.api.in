...
import { teacherRegistrationSchema, userRegisterFormSchema, } from "../zod/userRegisterFormSchema.js";
import { token } from "../helpers/token.js";
import { admin } from "../db/schema/admin.js";
import { teachers } from "../db/schema/teachers.js";
class AuthController {
    async getUserLoginCredentials({ email, password, }) {
        let responseResult;
        ...
        const hashedPassword = await bcrypt.compare(`${password}`, checkUserInDb[0]?.user_password);
        if (!hashedPassword) {
                ...
                if (getTeacherDetails?.length > 0) {
                    ...
                    if (insertRefreshToken?.length > 0) {
                        const teacherDataReturning = {
                            ...
                            teacher_technicalities: getTeacherDetails[0]?.teacher_technicalities,
                            role: "TEACHER",
                        };
                        statusCodeForNoData = StatusCodes.OK;
                        statusCodeMessageForData = getStatusMessage(statusCodeForNoData);
    ...
    async registerNewUser({ name, email, password, branchName, collegeId, domainId, rollNo, courseId, }) {
        const hashedPassword = await bcrypt.hash(`${password}`, Number(process.env.HASH_PASSWORD));
        ...
        const checkUserInDb = await db
            .select()
            .from(users)
            .where(and(eq(users.user_email, email), eq(users.user_password, hashedPassword)));
        ...
        if (checkUserInDb.length > 0) {
            ...
            const insertStudent = await db.insert(students).values({
                ...
                branch_name: branchName,
                student_roll_no: Number(String(studentCollegeId?.[0]) +
    ...
    async registerNewTeacher({ fullName, emailAddress, password, assignedCourseId, experience, technicalSkills, }) {
        let results;
        let statusCode;
        ...
        if (!checkAppErrorForTeacher?.success) {
            dataVariables = checkAppErrorForTeacher?.error?.issues?.map((eachError) => ({
                key: eachError.path[0],
                message: eachError?.message,
            }));
