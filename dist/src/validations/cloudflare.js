...
export async function createLiveInput(title) {
...
export async function getViewerCount(liveInputId) {
    const res = await fetch(`https://api.cloudflare.com/client/v4/accounts/${CF_ACCOUNT_ID}/stream/live_inputs/${liveInputId}/videos`, {
...
export async function checkIsStreamLive(liveInputId) {
    const res = await fetch(`https://api.cloudflare.com/client/v4/accounts/${CF_ACCOUNT_ID}/stream/live_inputs/${liveInputId}`, {
        headers: { Authorization: `Bearer ${CF_API_TOKEN}` },
    });
    const data = await res.json();
...
export async function getRecordingUrl(liveInputId) {
    const res = await fetch(`https://api.cloudflare.com/client/v4/accounts/${CF_ACCOUNT_ID}/stream/live_inputs/${liveInputId}/videos`, {
        headers: { Authorization: `Bearer ${CF_API_TOKEN}` },
    });
    const data = await res.json();
